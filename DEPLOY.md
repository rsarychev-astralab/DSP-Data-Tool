...
3. Render подхватит `render.yaml` или укажите вручную:
   ...
   - **Health check:** `/health`
   - **Env:** `DADATA_API_KEY` (проверка юрлиц), `DSP_BASIC_USER` и `DSP_BASIC_PASSWORD` (обязательно на проде)
...
```bash
cd "/Users/lucsijsotrudniknasvete/Documents/Projects/DSP-Data-Tool"
